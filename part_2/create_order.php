...
function shortestPath($a, $b,array $graph,$i,array $cost,array $g,array $cos,$city,$res,$mon,$temp) {
	//$this->graph = $graph;   
...
if ( $cost[$i-1]< $S[$b][1]){
	...
	if ($i-1>1){
		...
		while ($y<$l-1){
      	...
      	$b=	$b+$g[$temp[$y]][$temp[$y+1]];
		$y++;
		}
	   
        if ($a<$b){
			
			echo "To kostos einai: ".$a."<br>";	
		}
	    else{
           echo "To kostos einai: ".$b."<br>";	
		}
   ...
   // echo "<br />The length is ".$S[$b][1];
	...
	$cost[$i]=$S[$b][1];
	//echo $i;
    return shortestPath($a, $b,$result,$i+1,$cost,$g,$cos,$city,$res,$mon,$temp);
	
}
   }

shortestPath($or,$ds,$graph,1,$cost,$g,$cos,$city,$res,$mon,$temp);
